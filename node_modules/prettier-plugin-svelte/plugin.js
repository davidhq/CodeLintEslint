...
function print(path, options$$1, print) {
  ...
  switch (node.type) {
    ...
    case 'Title': {
      ...
      const isSelfClosingTag = isEmpty && (!options$$1.svelteStrictMode || node.type !== 'Element' || SELF_CLOSING_TAGS.indexOf(node.name) !== -1);
      return group(
        concat([
          '<',
          node.name,
          indent(
            group(
              concat([
                node.type === 'InlineComponent' && node.expression ? concat([line, 'this=', open, printJS(path, print, 'expression'), close]) : '',
                ...path.map(childPath => childPath.call(print), 'attributes'),
                options$$1.svelteBracketNewLine ? dedent(isSelfClosingTag ? line : softline) : ''
              ])
            )
          ),
    ...
    case 'Options':
    case 'Body':
      return group(concat(['<', node.name, indent(group(concat(path.map(childPath => childPath.call(print), 'attributes')))), ' />']));
    case 'Identifier':
      return node.name;
    case 'Attribute': {
      const hasLoneMustacheTag = node.value !== true && node.value.length === 1 && node.value[0].type === 'MustacheTag';
      let isAttributeShorthand = node.value !== true && node.value.length === 1 && node.value[0].type === 'AttributeShorthand';
      // Convert a={a} into {a}
      if (hasLoneMustacheTag) {
        const expression = node.value[0].expression;
        isAttributeShorthand = expression.type === 'Identifier' && expression.name === node.name;
      }
    ...
    case 'IfBlock': {
      const def = ['{#if ', printJS(path, print, 'expression'), '}', indent(printChildren(path, print))];
      if (node.else) {
    ...
    case 'EachBlock': {
      const def = ['{#each ', printJS(path, print, 'expression'), ' as ', printJS(path, print, 'context')];
      if (node.index) {
        def.push(', ', node.index);
      }
      if (node.key) {
    ...
    case 'AwaitBlock': {
      ...
      return group(
        concat([
          group(concat(['{#await ', printJS(path, print, 'expression'), ' then ', node.value ? node.value : '', '}'])),
          indent(path.call(print, 'then')),
          '{/await}'
        ])
      );
    ...
    case 'EventHandler':
      return concat([
        ...
        'on:',
        node.name,
        node.modifiers && node.modifiers.length ? concat(['|', join('|', node.modifiers)]) : '',
        node.expression ? concat(['=', open, printJS(path, print, 'expression'), close]) : ''
      ]);
    case 'Binding':
      return concat([
        line,
        'bind:',
        node.name,
        node.expression.type === 'Identifier' && node.expression.name === node.name ? '' : concat(['=', open, printJS(path, print, 'expression'), close])
      ]);
    case 'Class':
      return concat([
        line,
    ...
    case 'DebugTag':
      return concat(['{@debug', node.identifiers.length > 0 ? concat([' ', join(', ', path.map(print, 'identifiers'))]) : '', '}']);
    case 'Ref':
    ...
    case 'Action':
      return concat([line, 'use:', node.name, node.expression ? concat(['=', open, printJS(path, print, 'expression'), close]) : '']);
    case 'Animation':
      return concat([line, 'animate:', node.name, node.expression ? concat(['=', open, printJS(path, print, 'expression'), close]) : '']);
    case 'RawMustacheTag':
...
function printChildren(path, print, surroundingLines = true) {
  ...
  flush();
  return concat([surroundingLines ? softline : '', join(hardline, childDocs), surroundingLines ? dedent(softline) : '']);
}
...
const {
  builders: { concat: concat$1, hardline: hardline$1, group: group$1, indent: indent$1 },
  utils: { removeLines }
} = prettier.doc;
...
function nukeLastLine(doc) {
  ...
  switch (doc.type) {
    case 'concat':
      ...
      if (end > -1) {
        return concat$1([...doc.parts.slice(0, end), nukeLastLine(doc.parts[end]), ...doc.parts.slice(end + 1)]);
      }
      break;
    case 'line':
      return '';
...
function embedTag(tag, path, print, textToDoc, node, inline) {
  ...
  let content = '';
  if (contentAttribute && Array.isArray(contentAttribute.value) && contentAttribute.value.length > 0) {
